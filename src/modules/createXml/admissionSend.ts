...
// Função auxiliar para salvar logs XML por estabelecimento
async function writeXmlLog(xml: string, fileName: string, estabelecimentoCode?: number) {
  try {
...
async function writeResumoLog(content: string, fileName: string, estabelecimentoCode?: number) {
  try {
    const folder = await makeDirectory()
    if (!folder) {
      console.error('Erro: Não foi possível obter o diretório de logs')
...
export async function admission(dataAtendimentoFromDatabase: any[], hospitalCode?: number) {
  ...
  for (const item of dataAtendimentoFromDatabase) {
    ...
    try {
      ...
      console.log(`Enviando atendimento ${item.CD_DTI_ATENDIMENTO} (${item.NR_ATENDIMENTO}) para hospital ${hospitalCode}`)
      
      // Envia o XML individual
      const response = await makeRequest(xml, hospitalCode!)
      console.log(`Resposta para atendimento ${item.CD_DTI_ATENDIMENTO}:`, response.status)
            ...
            if (sBody) {
              ...
              if (internacoesArray && internacoesArray.length > 0) {
                ...
                if (codigoAtendimento !== undefined && situacao !== undefined) {
                  ...
                  if (situacao === 'P') {
                    console.log(`Atendimento ${item.CD_DTI_ATENDIMENTO} - Erro`)
                    ...
                    console.log(`Atendimento ${item.CD_DTI_ATENDIMENTO} - Sucesso`)
                    await knex
                      .update({ TP_STATUS: 'T' })
                      .from(TBL_ATENDIMENTO)
                      .where('CD_DTI_ATENDIMENTO', item.CD_DTI_ATENDIMENTO)
      ...
      console.error(`Erro ao processar atendimento ${item.CD_DTI_ATENDIMENTO}:`, error)
      contErro++
...
==========================================
`
      await writeResumoLog(errorLog, `erro_atendimento_${item.CD_DTI_ATENDIMENTO}`, hospitalCode)
